...
export class ArticleGrpcClient {
  ...
  public getFeaturedArticles(request: GetFeaturedArticlesRequest): Promise<GetFeaturedArticlesResponse> {
    return new Promise((resolve, reject) => {
      this.client.GetFeaturedArticles(request, (error, response) => {
        error ? reject(error) : resolve(response);
      });
    });
  }
